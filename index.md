---
layout: home
permalink: /
permalink_name: /ACCUEIL
title: Accueil

detail_image: assets/img/port.png

---

<strong><center><font size="5"> PRESENTATION </font></center><strong/>

Je m'appelle Joseph Enkaoua et suis passionné d'informatique. Je suis diplômé d'un BTS SIO option SISR.

<center><img src="assets/img/profil.png"></center>

<strong><center><font size="4"> Parcours Professionnel </font></center></strong>

J'ai acquis dans le cadre de mon parcours tout au long de mes années d'études différentes compétences qui m'ont encouragés à continuer sur cette voie qu'est l'informatique.
J'ai d'abord eu un Baccalauréat Economique et Social avec mention assez Bien, puis je suis entré à l'ORT Montreuil pour avoir mon BTS SIO.
J'ai choisi de me spécialiser vers l'option SISR car c'est ce qui me correspond le plus en Informatique, c'est à dire comprendre comment fonctionne un réseau et le mettre en place, mais aussi savoir trouver des solutions adaptés à chaque types problèmes.

<br><strong><center><font size="4"> Compétences techniques </font></center></strong>

- Administration Réseau, Cisco IOS, Ubiquiti Unify, EdgeRouter
- Administration Linux, Windows server
- Administration Office 365
- Virtualisation VMware vSphere, VirtualBox
- Connaissances en solution cloud Microsoft Azure

<br><strong><center><font size="4"> Projet professionnel </font></center></strong>

je souhaiterais pouvoir entrer dans le millieu du travail dans lequel je vais pouvoir apprendre plus concrètement le réseau.
J'aimerais pouvoir pour accompagner les gens et les aider à se familiariser avec les nouvelles technologies, car elles ne cessent d'évoluer de nos jours et cela devient un outil indispensable pour tout types de travaux.
J'aimerais à l'avenir me spécialiser en cybersécurité en faisant des certifications directement en ligne par exemple en commançant par la certification CEH et puis Comptia+



